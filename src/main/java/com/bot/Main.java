...
public class Main {
	...
	private static String funcionalidades = new StringBuilder().append(tab).append("/tempo \n").append(tab)
			.append("/conselho \n").append(tab).append("/piada_Chuck_Norris \n").append(tab)
			.append("/ordenar \"Passe os números que você deseja ordenar \" \n").toString();
				...
				if (update.message() != null) {
					...
					try {
						if (update.message().text().toLowerCase().equals("/start")) {
							...
							ResponseEntity<ApiTranslateResponse> requestTradutorAPI = APIs
									.requestTradutorAPI(telegramRespostaConselho, token_rapid_key);
							...
							ResponseEntity<ApiJokeChuckNorrisResponse> requestJokeChuckNorrisAPI = APIs
									.requestJokeChuckNorrisAPI();
							String telegramRespostaPiadaChuckNorris = telegramRespostaPiadaChuckNorris(update,
									requestJokeChuckNorrisAPI);
							ResponseEntity<ApiTranslateResponse> requestTradutorAPI = APIs
									.requestTradutorAPI(telegramRespostaPiadaChuckNorris, token_rapid_key);
							...
							try {
								...
								sendResponse = bot.execute(new SendMessage(update.message().chat().id(),
										"Desculpe talvez minha funcionalidade de ordenação n�o esteja funcionando, certifique-se de que você tenha passado na frente do comando /ordenar apenas números com um espaço em branco entre cada número. Depois tente novamente se quiser. "));
							}

						} else {
							sendResponse = bot.execute(new SendMessage(update.message().chat().id(),
									"Desculpe eu não entendi, pode dizer novamente? "));

						}
					} catch (Exception e) {
						System.out.println(e);
						sendResponse = bot.execute(new SendMessage(update.message().chat().id(),
								"Desculpe estou com algum probleminha... Tente novamente mais tarde. "));
					}
...
//					System.out.println("Mensagem Enviada? " + sendResponse.isOk());
	...
	private static void telegramRespostaPadrao(Update update) {
		sendResponse = bot.execute(new SendMessage(update.message().chat().id(),
				"Bem vindo usuário " + nomeDoUsuario(update) + " ao BOT"));
		sendResponse = bot
	...
	private static String telegramRespostaConselho(Update update,
			ResponseEntity<ApiAdviceResponse> requestConselhoAPI) {
	...
	private static void telegramRespostaConselhoTradutor(Update update,
			ResponseEntity<ApiTranslateResponse> requestConselhoTradutorAPI) {
	...
	private static String telegramRespostaPiadaChuckNorris(Update update,
			ResponseEntity<ApiJokeChuckNorrisResponse> requestJokeChuckNorrisAPI) {
	...
	private static void telegramRespostaPiadaChuckNorrisTradutor(Update update,
			ResponseEntity<ApiTranslateResponse> requestTradutorAPI) {
